...
export default async function HomeworkPage() {
  ...
  const { data } = await supabase
    .from('homework')
    .select('id, title, description, due_date, status, coach_feedback, coach_feedback_at')
    .eq('user_id', user.id)
    .order('due_date', { ascending: true, nullsFirst: false })

  const pending = (data ?? []).filter((h: Homework) => h.status === 'pending' || h.status === 'in_progress')
  const submitted = (data ?? []).filter((h: Homework) => h.status === 'submitted' || h.status === 'reviewed')
          ...
          <div className="flex flex-col gap-4">
            {pending.map((h: Homework) => (
              <div key={h.id} className="bg-white rounded-3xl p-5 border border-line shadow-card">
                ...
                {h.description && <p className="font-inter text-sm text-muted leading-relaxed mb-3">{h.description}</p>}
