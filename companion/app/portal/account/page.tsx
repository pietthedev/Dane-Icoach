...
interface Profile {
  ...
  pref_learn_from_ratings: boolean
  pref_auto_summarise: boolean
  pref_cross_session_memory: boolean
  pref_email_digest: boolean
  pref_push_notifications: boolean
  pref_quote_of_day: boolean
  consent_marketing: boolean
  consent_data_training: boolean
}

const DEFAULT: Profile = {
  full_name: '', email: '', phone: '', timezone: 'Africa/Johannesburg', language_preference: 'en',
  pref_learn_from_ratings: true, pref_auto_summarise: true, pref_cross_session_memory: false,
  pref_email_digest: true, pref_push_notifications: true, pref_quote_of_day: true,
  consent_marketing: false, consent_data_training: false,
}
...
export default function AccountPage() {
  ...
  const field = (id: keyof Profile, label: string, type = 'text', readOnly = false) => (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="font-inter font-semibold text-plum-dark text-xs uppercase tracking-wide">{label}</label>
      <input id={id} type={type} value={String(profile[id])} readOnly={readOnly}
        onChange={e => !readOnly && setProfile(p => ({ ...p, [id]: e.target.value }))}
        className={`font-inter text-sm text-ink border border-mist rounded-2xl px-4 py-3 bg-cloud focus:outline-none focus:border-plum transition-colors ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}`} />
    </div>
  )
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-line shadow-card">
          ...
          <div className="flex flex-col gap-4">
            {toggle('pref_learn_from_ratings', 'Learn from my ratings', 'Personalise responses based on how I rate sessions')}
            {toggle('pref_auto_summarise', 'Auto-summarise conversations')}
            {toggle('pref_cross_session_memory', 'Cross-session memory', 'Remember context from previous conversations')}
            {toggle('pref_email_digest', 'Weekly email digest')}
            {toggle('pref_push_notifications', 'Push notifications')}
        ...
        <div className="bg-white rounded-3xl p-6 border border-line shadow-card">
          ...
          <div className="flex flex-col gap-4">
            {toggle('consent_marketing', 'Marketing communications', 'Coaching tips and platform updates by email')}
            {toggle('consent_data_training', 'Anonymised data for platform improvement', 'Your conversations may contribute to improving the AI, in anonymised form')}
          </div>
        </div>
